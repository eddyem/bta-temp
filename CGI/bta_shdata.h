...
#define  SENSOR_B   (INPUT_B  <<8)   /* флаги работы (внешних?) метео-датчиков */
...
static void get_shm_block( struct SHM_Block *sb, int server) {
   ...
   fprintf(stderr,"Create&attach shared memory segment '%s' %dbytes at %x \n",
	   sb->key.name, sb->size, (unsigned int)sb->addr);
...
unsigned long cmd_src_ip=0;  /* IP-адр. источника для ОДНОЙ след.команды */
#else
extern int snd_id;
...
static void set_cmd_src(unsigned long ip, int pid) {
    cmd_src_pid = pid;
    cmd_src_ip = ip;
}
